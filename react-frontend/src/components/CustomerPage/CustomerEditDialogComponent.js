...
const CustomerCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            ...
            customerContact: _entity.customerContact,
        };
    ...
    return (
        <Dialog header="Edit Info" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="customer-edit-dialog-component">
                <div>
                <p className="m-0">Address:</p>
                <InputText className="w-full mb-3" value={_entity?.customerAddress} onChange={(e) => setValByKey("customerAddress", e.target.value)}  />
            </div>
            <div>
                <p className="m-0">Contact Number:</p>
                <InputText className="w-full mb-3" value={_entity?.customerContact} onChange={(e) => setValByKey("customerContact", e.target.value)}  />
            </div>
